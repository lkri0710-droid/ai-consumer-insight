/**
 * AI分析共享逻辑
 * 本文件同时被Next.js API Route和Cloudflare Pages Function引用，
 * 必须确保不依赖任何Node.js专有API（process.env等），
 * 只包含纯函数和常量，以兼容Cloudflare Workers运行时。
 */

export const SYSTEM_PROMPT = `你是一个资深的市场研究和消费者洞察分析专家。你的任务是根据用户提供的消费者反馈文本，进行深度分析并输出结构化的JSON结果。
